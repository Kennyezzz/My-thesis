...
# for(i in 0:1){
i = 0
data<-read.csv('Hourly_Air_Data_Nanzi.csv',stringsAsFactors = T)
df = data[,!(names(data) %in% c('Year','hour','MonitorDate'))]
x = data[,!(names(data) %in% c('hour','AQI','MonitorDate'))]
y = data$AQI
...
write.csv(evimp(MARS, trim=F), file =glue("importance/AQI_MARS_importance(R)_t{i}.csv"), row.names = T)
...
AQI.rf<-randomForest(AQI~., data=df,mtry = cv_rf$bestTune$mtry, importance=T)
                     # ntree=30, proximity=T, na.action=na.omit)
AQI.rf$importanceSD
rf_pred<- predict(AQI.rf,x)
(rf_RMSE<-rmse(y,rf_pred))
(rf_MAE<-mae(y,rf_pred))
(rf_MAPE<-mape(y,rf_pred))
write.csv(AQI.rf$importanceSD, file =glue("importance/AQI_RF_importance(R)_t{i}.csv"), row.names = T)
####т程ㄎ把计
####################
#######XGB##########
####################
# train_matrix <- sparse.model.matrix(AQImean ~ .-1, data = df)
xgb.train = xgb.DMatrix(data = as.matrix(df[,-19]),label = as.matrix(df$AQI))
###########xgb
...
xgb.pred<-predict(xgb,as.matrix(df[,-19], reshape = T))
...
write.csv(xgb_imp, file =glue("importance/AQI_XGB_importance(R)_t{i}.csv"), row.names = T)
